...
ripley_ROI1_tumor_univariate3 <- ripley_ROI1_tumor_univariate %>% 
  inner_join(., rois_clinical %>% 
            ...
            by= c("suid", "image_tag")) %>% 
  filter(marker != "DAPI (DAPI) Positive") %>% 
...
# ripley_ROI1_tumor_univariate3 %>% 
#   group_by(r, marker) %>% 
#   summarize(average_degree_perm = mean(degree_of_clustering_permutation, na.rm = TRUE)) %>% 
#   ungroup() %>% 
#   ggplot(aes(x= r, y= average_degree_perm, color= marker))+
#   geom_line()
...
ripley_ROI1_tumor_univariate3 %>% 
  ...
  summarize(average_degree_perm = mean(degree_of_clustering_permutation, na.rm = TRUE)) %>%
  ungroup() %>%
  ggplot(aes(x= r, y= average_degree_perm, color= suid))+
  geom_line()+
  ggtitle("CD3")+
...
We use a radius of 150 and look for T cell markers
```{r radius filter}
...
## Explore Cell Abundance/Spatial
...
ripley_ROI1_tumor_univariate_final %>% 
  ...
  select(refage, refage_cat, stage, BMI_recent, BMI_recent_grp,
         ...
         dblkstat_treat, 
         degree_of_clustering_permutation,
         vitalstatus, abundance_spatial_cat,
         race) %>% 
  tbl_summary(by = race,
...
ripley_ROI1_tumor_univariate_final %>% 
  # filter(marker == "CD3+") %>% 
  filter(compartment == "Tumor") %>% 
  ggplot(aes(x= abundance_spatial_cat, 
             y= degree_of_clustering_permutation,
             color= abundance_spatial_cat))+
  geom_boxplot()+
...
tbl_merge(list(tbl1_1, tbl2_1, tbl3_1), tab_spanner = c("CD3+ cat", "CD3+ CD8+ cat", "**CD3+ FOXP3+ cat**"))
```
PH assumption
```{r}
coxph(Surv(time = tbl1$timelastfu,
                   event = tbl1$os_event) ~ abundance_spatial_cat + refage + race + stage,
      data = tbl1)  %>%
  cox.zph()
coxph(Surv(time = tbl2$timelastfu,
                   event = tbl2$os_event) ~ abundance_spatial_cat + refage + race + stage,
      data = tbl2) %>%
  cox.zph()
coxph(Surv(time = tbl3$timelastfu,
                   event = tbl3$os_event) ~ abundance_spatial_cat + refage + race + stage,
      data = tbl3) %>%
  cox.zph()
```

# Exploratory data by Race
## Explore Cell Abundance/Spatial
...
surv_data <- ripley_ROI1_tumor_univariate_final %>% 
  ...
  filter(compartment == "Tumor") %>% 
  filter(marker == "CD3+") %>% 
  rename(cat = abundance_spatial_cat)

ggsurvplot(survfit(Surv(timeint_fu, os_event) ~ cat + race, 
                   data=surv_data),
           title = "OS Analysis",
           font.main = c(20, "bold", "black"),
           font.x = c(18, "bold", "black"),
...
# KM Abundance-Spatial Categories
...
surv_data <- ripley_ROI1_tumor_univariate_final %>% 
  ...
  filter(compartment == "Tumor") %>% 
  filter(marker == "CD3+") %>% 
  filter(abundance_spatial_cat == "Absence")

ggsurvplot(survfit(Surv(timeint_fu, os_event) ~ race, 
                   data=surv_data),
           title = "OS Analysis",
           font.main = c(20, "bold", "black"),
           font.x = c(18, "bold", "black"),
...
surv_data <- ripley_ROI1_tumor_univariate_final %>% 
  ...
  filter(compartment == "Tumor") %>% 
  filter(marker == "CD3+") %>% 
  filter(abundance_spatial_cat == "High A/High S")
...
surv_data <- ripley_ROI1_tumor_univariate_final %>% 
  ...
  filter(compartment == "Tumor") %>% 
  filter(marker == "CD3+") %>% 
  filter(abundance_spatial_cat == "High A/Low S")
...
surv_data <- ripley_ROI1_tumor_univariate_final %>% 
  ...
  filter(compartment == "Tumor") %>% 
  filter(marker == "CD3+") %>% 
  filter(abundance_spatial_cat == "Low A/High S")
...
surv_data <- ripley_ROI1_tumor_univariate_final %>% 
  ...
  filter(compartment == "Tumor") %>% 
  filter(marker == "CD3+") %>% 
  filter(abundance_spatial_cat == "Low A/Low S")
