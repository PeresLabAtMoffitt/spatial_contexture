---
title: "Investigate Ripley in Univariate Spatial Contexture - Total cell - radius 150"
...
<!-- We are removing patients who doesn't have at least 1 spatial value for CD3+ intratumoral.    -->
<!-- Please can you review these patients (17 patients would be excluded) -->
```{r, rows.print=285}
# ripley_ROI1_overall_univariate5 <- ripley_ROI1_overall_univariate4 %>% 
#   mutate(has_a_cd3_value = case_when(
#     marker == "CD3+" &
#       annotation == "Intratumoral" &
#       !is.na(degree_of_clustering_permutation)              ~ "has value"
#   )) %>% 
#   group_by(suid) %>% 
#   fill(has_a_cd3_value, .direction = "updown") %>% 
#   ungroup() 
# 
# ripley_ROI1_overall_univariate5 %>% 
#   filter(is.na(has_a_cd3_value)) %>% 
#   select(suid, marker, annotation, theoretical_csr, degree_of_clustering_permutation)
# 
# 
# 
# ripley_ROI1_overall_univariate4 <- ripley_ROI1_overall_univariate5 %>% 
#   filter(!is.na(has_a_cd3_value))
```
...
tbl3 <- ripley_ROI1_overall_univariate_final %>% 
  filter(compartment == "Total") %>% 
  ...
  bold_labels() %>% 
  add_p() %>% bold_p(t= .05) %>% add_overall()
tbl5 <- ripley_ROI1_overall_univariate_final %>% 
  filter(compartment == "Total") %>% 
...
```{r, fig.height = 7}
surv_data <- ripley_ROI1_overall_univariate_final %>% 
  filter(annotation == "Intratumoral") %>% 
  filter(compartment == "Total") %>% 
  filter(marker == "CD3+")
